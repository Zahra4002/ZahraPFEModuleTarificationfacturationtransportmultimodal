...
{
    public class DeleteInvoiceLineCommandHandler : IRequestHandler<DeleteInvoiceLineCommand, ResponseHttp>
    {
        ...
        public async Task<ResponseHttp> Handle(DeleteInvoiceLineCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (invoice.Status != InvoiceStatus.Brouillon)
                {
                    return new ResponseHttp
                    {
                        ...
                        Fail_Messages = $"Impossible de supprimer une ligne d'une facture avec le statut {invoice.Status}"
                    };
                }

                // 3️⃣ Récupérer la ligne à supprimer
